    ...
    return (<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm">
            <div className="max-w-lg w-full mx-4 border border-[#1E293B] bg-[#0B101A] p-8">
                ...
                <div className="space-y-3 mb-6">
                    {availableGroups.map((group) => (<button key={group.id} onClick={() => handleSelectGroup(group.id)} className="w-full p-4 border border-[#334155] hover:border-blue-500/50 bg-[#0F172A] hover:bg-[#1E293B] transition-all text-left group">
                ...
                <div className="text-center">
                    <button onClick={handleCreateGroup} className="text-xs font-mono text-gray-500 hover:text-gray-300 underline transition-colors">
                        Manage Groups →
                    </button>
                </div>
            </div>
...
export function GroupSelectorDropdown() {
    ...
    if (availableGroups.length <= 1) {
        return currentGroup ? (<div className="text-xs font-mono text-gray-500 uppercase">
                {currentGroup.name}
            </div>) : null;
    }
    return (<div className="relative group">
            ...
            <div className="absolute top-full left-0 mt-1 w-64 border border-[#334155] bg-[#0B101A] opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50 shadow-xl">
                {availableGroups.map((group) => (<button key={group.id} onClick={() => selectGroup(group.id)} className={`w-full px-4 py-3 text-left hover:bg-[#1E293B] transition-colors border-b border-[#1E293B] last:border-b-0 ${group.id === selectedGroupId ? "bg-blue-900/20" : ""}`}>
                        <span className="text-xs font-mono text-gray-400 uppercase">
                            {group.name}
                        </span>
                        {group.id === selectedGroupId && (<span className="ml-2 text-blue-400">✓</span>)}
                    </button>))}
            </div>
        </div>);
}
