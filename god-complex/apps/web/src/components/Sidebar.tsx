...
function SidebarItem({ href, label, icon: Icon }: {
    ...
    return (<Link href={href} className="group flex items-center gap-4 py-3 relative">
            {isActive && (<motion.div layoutId="active-pill" className="absolute left-0 w-[2px] h-full bg-blue-500 shadow-[0_0_10px_2px_rgba(59,130,246,0.5)]"/>)}
...
function SectionHeader({ label }: {
    ...
    return (<div className="px-6 py-4 mt-4">
...
export function Sidebar() {
    ...
    const { availableGroups, selectedGroupId, loading, stale } = useDashboardContext();
    const currentGroup = availableGroups.find(g => g.id === selectedGroupId);
    const hasMultipleGroups = availableGroups.length > 1;
    return (<aside className="fixed left-0 top-0 bottom-0 w-64 bg-[#050810] border-r border-[#1E293B] z-50 flex flex-col hidden md:flex">
            ...
            {loading ? (<div className="px-6 py-4 border-b border-[#1E293B] bg-[#0B101A]/50">
                ...
                </div>) : currentGroup ? (<div className="px-6 py-4 border-b border-[#1E293B] bg-[#0B101A]/50">
                    <div className="flex items-center justify-between">
                        <div className="flex flex-col">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                {stale && (<div className="w-2 h-2 rounded-full bg-yellow-500 animate-pulse" title="Data may be stale"></div>)}
                            </div>
                        </div>
                        {hasMultipleGroups && (<div className="w-5 h-5 rounded-full bg-blue-900/30 border border-blue-500/50 flex items-center justify-center">
                                <span className="text-[10px] text-blue-400 font-bold">
                                    {availableGroups.length}
                                </span>
                            </div>)}
                    </div>
                </div>) : !loading && availableGroups.length === 0 ? (<div className="px-6 py-4 border-b border-[#1E293B] bg-[#0B101A]/50">
                    <div className="flex flex-col">
                        <span className="text-[9px] text-gray-600 uppercase tracking-widest mb-2">
                            No Groups
                        </span>
                        <p className="text-[10px] text-gray-500 mb-2">
                            Join a group to get started
                        </p>
                        <Link href="/groups" className="text-[10px] text-blue-400 hover:text-blue-300 underline">
                            Browse Groups →
                        </Link>
                    </div>
                </div>) : null}
            ...
            <div className="p-6 border-t border-[#1E293B] space-y-3">
                <button onClick={logout} className="w-full py-2 px-4 bg-red-900/20 hover:bg-red-900/30 border border-red-900/50 text-red-500 text-[10px] font-mono tracking-widest uppercase transition-colors">
                    LOGOUT
                </button>
