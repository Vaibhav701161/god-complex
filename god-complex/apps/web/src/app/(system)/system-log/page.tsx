...
function SourceBadge({ source }: {
    ...
    return (<span className={`inline-block px-2 py-1 text-[9px] font-mono uppercase tracking-wider border rounded ${colors.bg} ${colors.border} ${colors.text}`}>
            {source}
        </span>);
}
export default function SystemLog() {
    ...
    return (<main className="min-h-screen bg-[#0a0e14] pb-32 p-6 md:p-12 font-sans">
            ...
            {error && (<div className="mb-6 p-4 border border-red-900/50 bg-red-950/20">
                    <p className="text-sm font-mono text-red-400">{error}</p>
                </div>)}

            
            <div className="flex gap-4 mb-8 flex-wrap">
                <select onChange={(e) => handleDateRangeChange(e.target.value)} className="bg-[#050810] border border-[#1E293B] text-gray-400 text-xs font-mono p-2 rounded-sm focus:outline-none focus:border-blue-900">
                    <option>All Dates</option>
                    <option>Last 24 Hours</option>
                    <option>Last 7 Days</option>
                    <option>This Month</option>
                </select>
                <select onChange={(e) => handleSourceChange(e.target.value)} className="bg-[#050810] border border-[#1E293B] text-gray-400 text-xs font-mono p-2 rounded-sm focus:outline-none focus:border-blue-900">
                ...
                <select value={filters.action || ""} onChange={(e) => setFilters(prev => ({ ...prev, action: e.target.value || undefined }))} className="bg-[#050810] border border-[#1E293B] text-gray-400 text-xs font-mono p-2 rounded-sm focus:outline-none focus:border-blue-900">
                ...
                {filters.correlationId && (<div className="flex items-center gap-2 bg-blue-950/20 border border-blue-900/50 px-3 py-2 rounded-sm">
                        <span className="text-xs text-blue-400 font-mono">Correlation: {filters.correlationId.slice(0, 12)}...</span>
                        <button onClick={() => setFilters(prev => ({ ...prev, correlationId: undefined }))} className="text-blue-400 hover:text-blue-300">
                            ✕
                        </button>
                    </div>)}
            </div>
            ...
            {logs.length === 0 && !loading && (<div className="border border-dashed border-[#1E293B] p-12 text-center">
            ...
            {logs.length > 0 && (<div className="border-t border-[#1E293B]">
                ...
                return (<div key={log.id}>
                                ...
                                <div className="grid grid-cols-12 py-3 border-b border-[#1E293B]/30 hover:bg-[#0B101A]/50 transition-colors cursor-pointer" onClick={() => setExpandedLogId(isExpanded ? null : log.id)}>
                                    ...
                                    <div className="col-span-1 px-4 text-right">
                                        <svg className={`w-4 h-4 text-gray-600 inline-block transition-transform ${isExpanded ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                {isExpanded && (<div className="border-t border-[#1E293B] bg-[#050810] p-6">
                                        ...
                                        <div className="grid grid-cols-2 gap-4 mb-4">
                                            ...
                                            <div>
                                                ...
                                                {log.correlationId ? (<button onClick={(e) => {
                                e.stopPropagation();
                                handleCorrelationIdClick(log.correlationId!);
                            }} className="text-sm text-blue-400 font-mono hover:text-blue-300 underline">
                                                        {log.correlationId}
                                                        {relatedCount > 1 && (<span className="ml-2 text-xs text-gray-600">({relatedCount} related)</span>)}
                                                    </button>) : (<span className="text-sm text-gray-400">N/A</span>)}
                                            </div>
                                        </div>
