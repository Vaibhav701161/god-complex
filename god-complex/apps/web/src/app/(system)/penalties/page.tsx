...
function StatusBadge({ status, verdict }: {
    ...
    const colorMap: Record<PenaltyStatus, {
        border: string;
        text: string;
        bg: string;
    }> = {
    ...
    return (<div className={`inline-flex items-center gap-2 px-3 py-1 border ${colors.border} ${colors.bg}`}>
            <span className={`text-xs font-mono tracking-widest uppercase ${colors.text}`}>
                {status}
            </span>
            {status === 'RESOLVED' && verdict && (<span className={`text-[10px] font-mono tracking-widest uppercase ${verdict === 'REVERSED' ? 'text-green-400' : 'text-red-400'}`}>
                    • {verdict}
                </span>)}
        </div>);
}
function AppealModal({ penaltyId, onClose, onSubmit }: {
    penaltyId: string;
    onClose: () => void;
    ...
    return (<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm">
            <div className="max-w-2xl w-full mx-4 border border-[#1E293B] bg-[#0B101A] p-8">
                ...
                {error && (<div className="mb-4 p-3 border border-red-900/50 bg-red-950/20">
                        <p className="text-sm font-mono text-red-400">{error}</p>
                    </div>)}

                
                <div className="mb-6">
                    <textarea value={reason} onChange={(e) => setReason(e.target.value)} placeholder="Provide a detailed explanation for your appeal..." className="w-full h-40 p-4 bg-[#0F172A] border border-[#334155] text-gray-300 font-mono text-sm resize-none focus:outline-none focus:border-blue-500/50" disabled={isSubmitting}/>
                    <div className="flex justify-between items-center mt-2">
                        <span className="text-xs text-gray-600 font-mono">
                            {reason.length < 20 ? (<span className="text-yellow-500">Minimum 20 characters required</span>) : (<span className="text-green-500">✓ Valid length</span>)}
                        </span>
                        <span className={`text-xs font-mono ${reason.length > 500 ? 'text-red-500' : 'text-gray-500'}`}>
                            {reason.length} / 500
                        </span>
                    </div>
                </div>
                ...
                <div className="flex gap-3">
                    <button onClick={handleSubmit} disabled={isSubmitting || reason.trim().length < 20 || reason.length > 500} className="flex-1 px-6 py-3 bg-blue-600 hover:bg-blue-500 text-white font-bold tracking-[0.2em] text-sm uppercase transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                        {isSubmitting ? "SUBMITTING..." : "SUBMIT APPEAL"}
                    </button>
                    <button onClick={onClose} disabled={isSubmitting} className="px-6 py-3 border border-[#334155] hover:border-gray-500 text-gray-400 hover:text-gray-300 font-bold tracking-[0.2em] text-sm uppercase transition-colors">
                        CANCEL
                    </button>
                </div>
            </div>
...
export default function PenaltiesPage() {
    const { user, userLoading, groupId, currentMonth, loading: contextLoading } = useDashboardContext();
    ...
    const now = new Date();
    const overduePenalties = penalties.filter(p => p.status === 'PENDING' && new Date(p.dueDate) < now);
    ...
    return (<main className="min-h-screen bg-[#0a0e14] pb-32">
            ...
            <div className="max-w-7xl mx-auto px-6 md:px-12 py-12">
                ...
                {overduePenalties.length > 0 && (<div className="border-2 border-red-500/50 bg-red-950/10 p-6 mb-6">
                ...
                {actionError && (<div className="mb-6 p-4 border border-red-900/50 bg-red-950/20">
                        <p className="text-sm font-mono text-red-400">{actionError}</p>
                        <button onClick={() => setActionError(null)} className="text-red-600 hover:text-red-400 font-mono text-xs mt-2">
                            [DISMISS]
                        </button>
                    </div>)}

                
                {penalties.length === 0 ? (<div className="border border-dashed border-[#1E293B] p-12 text-center">
                    ...
                    </div>) : (<div className="border border-[#1E293B] bg-[#0B101A]">
                ...
                return (<div key={penalty.id} className={`grid grid-cols-12 gap-4 p-4 border-b border-[#1E293B] last:border-b-0 ${isOverdue ? 'bg-red-950/5' : 'hover:bg-[#0F172A]'} transition-colors`}>
                                    ...
                                    <div className="col-span-3 flex items-center gap-2">
                                        {penalty.status === 'PENDING' && (<>
                                                <button onClick={() => handleMarkComplete(penalty.id)} disabled={isLoading} className="px-3 py-1 bg-green-900/30 hover:bg-green-900/50 border border-green-500/50 text-green-400 text-xs font-mono uppercase transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                                                    {isLoading ? 'MARKING...' : 'COMPLETE'}
                                                </button>
                                                <button onClick={() => setAppealingPenaltyId(penalty.id)} disabled={isLoading} className="px-3 py-1 bg-blue-900/30 hover:bg-blue-900/50 border border-blue-500/50 text-blue-400 text-xs font-mono uppercase transition-colors">
                                                    APPEAL
                                                </button>
                                            </>)}
                                        {penalty.status === 'FAILED' && (<button onClick={() => setAppealingPenaltyId(penalty.id)} disabled={isLoading} className="px-3 py-1 bg-blue-900/30 hover:bg-blue-900/50 border border-blue-500/50 text-blue-400 text-xs font-mono uppercase transition-colors">
                                                APPEAL
                                            </button>)}
                                        {penalty.status === 'APPEALED' && (<div className="text-xs font-mono text-gray-600 uppercase">
                                                Pending Review
                                            </div>)}
                                        {(penalty.status === 'COMPLETED' || penalty.status === 'RESOLVED') && (<div className="text-xs font-mono text-gray-600 uppercase">
                                                No Actions
                                            </div>)}
                                    </div>
                                </div>);
            ...
            {appealingPenaltyId && (<AppealModal penaltyId={appealingPenaltyId} onClose={() => setAppealingPenaltyId(null)} onSubmit={handleAppealSubmit}/>)}
        </main>);
}
