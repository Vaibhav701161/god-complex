...
export async function submitDailyGoals(userId: string, data: DailyGoalInput) {
    ...
    await prisma.$transaction(goals.map((g) => prisma.goal.create({
        data: {
            userId,
            groupId,
            date: new Date(date),
...
export async function getDailyReport(userId: string, groupId: string, date: string) {
    return prisma.goal.findMany({
        where: { userId, groupId, date: new Date(date) },
        include: { result: true },
    });
