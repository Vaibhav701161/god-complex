...
export function resolveGroupDay(
    nowUTC: Date,
    timezone: string,
    cutoffHour: number
): DayBoundaries {
    ...
    let opMonth = parts.month; // 1-indexed
    let opDay = parts.day;
    ...
    // Now calculate the UTC boundaries for this operational day.
    // Start: opDate at cutoffHour
    // End: (opDate + 1) at cutoffHour
...
export function getBoundariesForDate(
    dateLabel: string, // YYYY-MM-DD
    timezone: string,
    cutoffHour: number
): Omit<DayBoundaries, 'date'> {
    const [y, m, d] = dateLabel.split('-').map(Number);

    // Start: Date(y,m,d) at Cutoff
    const dayStartUTC = getUtcFromLocal(timezone, y, m, d, cutoffHour);

    // End: Date(y,m,d+1) at Cutoff
    // Note: UTC Date constructor handles overflow (Day 32 -> Next Month)
...
/**
 * Finds the UTC timestamp that corresponds to the given Local Time components.
 * Uses a convergence loop to handle offsets.
 */
function getUtcFromLocal(
    ...
    // Initial Guess: Treat inputs as UTC
    let guess = new Date(Date.UTC(year, month - 1, day, hour, 0, 0));

    for (let i = 0; i < 4; i++) {
        const local = getLocalParts(guess, timezone);

        // Calculate difference in ms between obtained local time and target local time
        // We treat the "Local" components as if they were UTC to compare numbers
        ...
        if (Math.abs(diff) < 1000) return guess; // Close enough
