...
router.post("/:penaltyId/complete", requireAuth, async (req, res) => {
...
router.post("/:penaltyId/appeal", requireAuth, async (req, res) => {
    try {
...
router.get("/consequences/:groupId", requireAuth, async (req, res) => {
    const data = await getPenaltyConsequences(req.user!.id, req.params.groupId);
    res.json(data);
});
